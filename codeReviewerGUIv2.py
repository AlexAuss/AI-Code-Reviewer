...
def on_button_click():
    ...
    populate_viewer_with_fixes(src)
...
def ensure_viewer():
    ...
    # Tags for diff-style rendering
    viewer_text.tag_configure("fix_semicolon", background="#e6ffed")            # highlight inserted ';'
    viewer_text.tag_configure("diff_add", foreground="#22863a")                 # green plus line
    viewer_text.tag_configure("diff_del", foreground="#cb2431")                 # red minus line
...
def populate_viewer_with_fixes(src: str):
    """Show ORIGINAL code and a diff-like CORRECTIONS section.
    In CORRECTIONS, output a '-' line with the original, and a '+' line with the
    corrected version (semicolon appended). Inserted semicolons are highlighted
    with a green background, and plus/minus lines get green/red foregrounds.
    """
    if viewer_text is None:
        return

    # Clear prior diff highlighting
    ...
    # 1) Original code section
    viewer_text.insert("end", "Original code:\n")
    if src:
        viewer_text.insert("end", src + "\n")
    else:
        viewer_text.insert("end", "(no input)\n")

    # 2) Corrections (diff-style)
    ...
    for raw in src.splitlines():
        ...
        if rstrip and not rstrip.endswith(";"):
            ...
            fixed = raw[:prefix_len] + ";" + raw[prefix_len:]

            # '-' deleted (original) line
            del_mark_start = viewer_text.index("end")
            viewer_text.insert("end", f"- {raw}\n")
            viewer_text.tag_add("diff_del", del_mark_start, f"{del_mark_start} lineend+1c")

            # '+' added (corrected) line
            add_mark_start = viewer_text.index("end")
            viewer_text.insert("end", f"+ {fixed}\n")
            viewer_text.tag_add("diff_add", add_mark_start, f"{add_mark_start} lineend+1c")

            # Highlight the inserted semicolon on the '+ ' line
            semi_start = f"{add_mark_start}+{2 + prefix_len}c"   # account for '+ '
            semi_end   = f"{add_mark_start}+{2 + prefix_len + 1}c"
            viewer_text.tag_add("fix_semicolon", semi_start, semi_end)

            # Explanation line
            viewer_text.insert("end", "You are missing a silicone\n")

    if not any_fixes:
        viewer_text.insert("end", "No lines required a trailing semicolon.\n")
